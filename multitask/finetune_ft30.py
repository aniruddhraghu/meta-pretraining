...
def get_subset_fold(data):
    include_start =  args.fold*10
    include_end =  args.fold*10 + 10
    seg = data[:,include_start:include_end]
...
def train(args, model, device, loader, optimizer):
    ...
    for step, batch in enumerate(loader):
        ...
        pred = model(batch)
        # HACK to just eval on last 10
        y = batch.go_target_downstream.view(-1, 40)
        y = get_subset_fold(y).view(pred.shape).to(torch.float64)
...
def eval(args, model, device, loader):
    ...
    for step, batch in enumerate(loader):
        ...
        y = batch.go_target_downstream.view(-1, 40)
        y_true.append(get_subset_fold(y).detach().cpu())
...
num_tasks = 10 #len(dataset[0].go_target_downstream)
...
os.makedirs(f"result-ft30-fold{args.fold}/finetune_seed" + str(args.runseed), exist_ok=True)
# torch.save(model.state_dict(), 'models/' + args.filename)

if not args.filename == "":
    with open(f"result-ft30-fold{args.fold}/finetune_seed" + str(args.runseed)+ "/" + args.filename, 'wb') as f:
        if args.split == "random":
