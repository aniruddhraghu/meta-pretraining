...
class GNN_graphpred(torch.nn.Module):
    ...
    def from_pretrained(self, model_file):
        ...
        if 'student_sd' not in sd:
            self.gnn.load_state_dict(torch.load(model_file))
        else:
            sd_red = {k[4:] : sd['student_sd'][k] for k in sd['student_sd'].keys() if 'gnn' in k}
            self.gnn.load_state_dict(sd_red)
...
class TaskWeight(torch.nn.Module):
    def __init__(self, num_classes):
        super(TaskWeight, self).__init__()
        self.num_classes=num_classes
        self.weights = torch.nn.parameter.Parameter(torch.zeros(num_classes))

    def forward(self):
        # form weights purely using classes.
        out_exw = 2*torch.sigmoid(self.weights)
        # print(out_exw.shape)
        return out_exw
