...
parser.add_argument('--savefol', type=str, default='exw-adamhyper-ft30tasks')
...
parser.add_argument('--fold', type=int, default=3, help='fold*10:fold*10+10 defines test tasks')

parser.add_argument('--checkpoint', type=str)
parser.add_argument('--ft_bn', action='store_true')
args = parser.parse_args()
args.savefol += f'-fold{args.fold}'
...
# HACK. reduce number of tasks by 10 for meta PT.
NUM_TASKS_FT = len(dataset[0].go_target_downstream) - 10
...
def get_save_path():
    modfol =  f"""ptlr{args.pretrain_lr}-ftlr{args.finetune_lr}-hyplr{args.hyper_lr}-warmup{args.warmup_epochs}-pt_steps{args.pretrain_steps}-ft_steps{args.finetune_steps}-neumann{args.neumann}-ft_bn{args.ft_bn}"""
...
def get_subset_fold(data):
    exclude_start = args.fold*10
    exclude_end = args.fold*10 + 10
    seg1 = data[:,:exclude_start]
    seg2 = data[:,exclude_end:]
...
def get_loss(student,head,teacher,  batch, ft):
    ...
    head_op = head(feats)
    if ft:
        # HACK HERE TO ONLY USE THE FIRST 30 TASKS
        y = batch.go_target_downstream.view(-1, 40)
        y = get_subset_fold(y).view(head_op.shape).to(torch.float64)
        ...
        y_loss_stud = torch.mean(comm*loss) + 0*torch.sum(head_op) * 0*torch.sum(loss)
...
def inner_loop_finetune(student, head, teacher, optimizer, train_dl, val_dl, num_steps):
    ...
    if args.ft_bn:
        ...
        student.eval() # BN train for only head unroll
    teacher.train()
...
def eval_student(student, head, dl):
    ...
    for step, batch in enumerate(tqdm(dl, desc="Iteration")):
        ...
        y_true.append(get_subset_fold(batch.go_target_downstream.view(-1,40).detach().cpu()))
